...
const BACKUP_PREFIX = "pai-backup-";
...
function listBackups(): BackupInfo[] {
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && (entry.name.startsWith(BACKUP_PREFIX) || entry.name.startsWith("claude-backup-"))) {
        ...
        backups.push({
          ...
          hasClaude: existsSync(join(backupPath, ".claude")),
          hasGemini: existsSync(join(backupPath, ".gemini")),
          hasSettings: existsSync(join(backupPath, ".claude", "settings.json")) || existsSync(join(backupPath, ".gemini", "settings.json")),
        });
      }
    }
  } catch (error) {
...
function createBackup(customName?: string): string | null {
  if (!existsSync(CLAUDE_DIR) && !existsSync(GEMINI_DIR)) {
    console.error("Error: Neither ~/.claude nor ~/.gemini directories exist. Nothing to backup.");
    return null;
  }
  ...
  try {
    if (existsSync(CLAUDE_DIR)) {
      console.log(`Backing up Claude: ${CLAUDE_DIR}`);
      cpSync(CLAUDE_DIR, join(backupPath, ".claude"), { recursive: true });
    }
    if (existsSync(GEMINI_DIR)) {
      console.log(`Backing up Gemini: ${GEMINI_DIR}`);
      cpSync(GEMINI_DIR, join(backupPath, ".gemini"), { recursive: true });
    }
    const size = getDirSize(backupPath);
...
function restoreBackup(backupName: string): boolean {
  ...
  const backupPath = backupName.startsWith("/")
    ? backupName
    : join(HOME, (backupName.startsWith(BACKUP_PREFIX) || backupName.startsWith("claude-backup-")) ? backupName : `${BACKUP_PREFIX}${backupName}`);
  ...
  if (existsSync(CLAUDE_DIR) || existsSync(GEMINI_DIR)) {
    ...
    // Remove current
    console.log("Removing current installation(s)...");
  ...
  try {
    if (existsSync(join(backupPath, ".claude"))) {
      console.log(`Restoring Claude: ${CLAUDE_DIR}`);
      cpSync(join(backupPath, ".claude"), CLAUDE_DIR, { recursive: true });
    } else if (existsSync(join(backupPath, "settings.json"))) {
      // Legacy backup format (root is .claude content)
      console.log("Legacy backup detected. Restoring to .claude");
      cpSync(backupPath, CLAUDE_DIR, { recursive: true });
    }

    if (existsSync(join(backupPath, ".gemini"))) {
      console.log(`Restoring Gemini: ${GEMINI_DIR}`);
      cpSync(join(backupPath, ".gemini"), GEMINI_DIR, { recursive: true });
    }
